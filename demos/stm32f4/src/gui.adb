...
package body GUI is
   ...
   protected State is
      function Get return GUI_State;
      procedure Set (Value : GUI_State);
   private
      pragma Priority (125);
      State : GUI_State;
   end State;
   ...
   protected body State is
      function Get return GUI_State is (State);

      procedure Set (Value : GUI_State) is
      begin
         State := Value;
      end Set;
   end State;
   ...
   is
      ...
      State : constant GUI_State := GUI.State.Get;
   begin
      if Clear then
      ...
      if State.Timestamp /= Timestamp then
         ...
         if State.Has_Fix then
            ...
            Put_Line
              (Text => (if State.Altitude = 0.0 then "L" else "H"),
               Font => BMP_Fonts.Font8x8,
               Line => 4);
         end if;
   ...
   procedure Show (Value : NMEA_0183.NMEA_Message) is
      ...
      procedure Data_Variant_C (Value : NMEA_0183.Data_Variant_C);

      State : GUI_State := GUI.State.Get;

      Timestamp : constant Natural := State.Timestamp;

      procedure Data_Variant_C (Value : NMEA_0183.Data_Variant_C) is
         use type NMEA_0183.Latitude;
         use type NMEA_0183.Longitude;
      begin
         if State.Has_Fix /= Value.Is_Valid then
            State.Has_Fix := Value.Is_Valid;
   ...
   begin
      case Value.Kind is
         ...
         when GPS_Data_Variant_C =>
            Data_Variant_C (Value.Data_Variant_C);
         when others =>
            null;
      end case;
   end Show;
